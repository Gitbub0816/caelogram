...
/** Named, not counted: later migrations must not shift what "before 0006" means. */
const WORKSPACES = migrations.indexOf("0006_workspaces.sql");
...
test("migration 0006 records every pre-workspace index and the runtime maps it onto its GitHub owner", async (t) => {
  const { mf, env, store, db, apply } = await harness(WORKSPACES);
  try {
    ...
    await apply(migrations.slice(WORKSPACES));
